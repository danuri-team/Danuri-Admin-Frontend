...
const CustomInput = ({label, value, onChange, valid}:CustomInputType) => {
    ...
    return(
        <div className="mb-[30px]">
            ...
            <div className={`${value.length > 0 && valid===false ? 'border-red-400' : 'border-gray-200'} flex border  rounded-xl p-[13px] w-full min-w-xs`}>
                <input 
                    ...
                    onChange={onChange} />
                {
                    value.length > 0 && (valid===true||valid===false) && (
                        <button className={`${value.length > 0 && valid===true ? 'text-danuri-500' : 'text-red-400'}`}>{valid === false ? <FaCircleExclamation /> : valid === true && <FaCircleCheck />}</button>
                    )
                }
