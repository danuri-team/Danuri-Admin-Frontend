...
import type { Rental } from "@/types/domains/rental";
...
export const useRentalPage = () => {
  ...
  // State
  const rentalTableHeader = useMemo(() => FIXED_TABLE_HEADERS, []);
  const [totalPages, setTotalPages] = useState<number>(0);
  const [tableData, setTableData] = useState<Rental[] | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      const res = await getSearchCompanyRental({ page, size });
      if (!res.pass) {
  ...
  const handleDeleteButton = useCallback(async () => {
    ...
    const res = await deleteRental({ rentalId: selectedRowId });
    if (res.pass) {
