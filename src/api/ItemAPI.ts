...
//아이템 생성
export const postCreateItem = async ({name, totalQuantity, status}:{name:string, totalQuantity:string, status:string}) => {
    try{
...
//아이템 수정
export const putUpdateItem = async ({itemId, name, totalQuantity, status}: {itemId:string, name:string, totalQuantity:string, status:string}) => {
    try{
